...
class MftMuonMatchingUtils
{
 private:
  ...
  inline o2::track::TrackParCovFwd propagateMFTtoMatchingPlane(bool usemch)
  {
    ...
    if (usemch) {
      ...
      float mft_theta = pi / 2. - std::atan(mfttrack.tgl());
 ...
 public:
  ...
  {
    auto mfttrack_on_matchingP = propagateMFTtoMatchingPlane(false);
    auto mfttrack_on_matchingP_useMCH = propagateMFTtoMatchingPlane(true);
    ...
    if (mUseMuonInfoToMFTExtrap) {
      dphiRaw = mfttrack_on_matchingP_useMCH.getPhi() - muontrack_on_matchingP.getPhi();
      dphi = TVector2::Phi_mpi_pi(dphiRaw);
      deta = mfttrack_on_matchingP_useMCH.getEta() - muontrack_on_matchingP.getEta();

      mDX = mfttrack_on_matchingP_useMCH.getX() - muontrack_on_matchingP.getX();
      mDY = mfttrack_on_matchingP_useMCH.getY() - muontrack_on_matchingP.getY();
      mDPt = mfttrack_on_matchingP_useMCH.getPt() - muontrack_on_matchingP.getPt();
      mDTanl = mfttrack_on_matchingP_useMCH.getTanl() - muontrack_on_matchingP.getTanl();
      mDPhi = dphi;
      mDEta = deta;

      mMatchingMFTX = mfttrack_on_matchingP_useMCH.getX();
      mMatchingMFTY = mfttrack_on_matchingP_useMCH.getY();
      mMatchingMFTEta = mfttrack_on_matchingP_useMCH.getEta();
      mMatchingMFTPhi = mfttrack_on_matchingP_useMCH.getPhi();
      mMatchingMFTTanl = mfttrack_on_matchingP_useMCH.getTanl();
    } else {
      dphiRaw = mfttrack_on_matchingP.getPhi() - muontrack_on_matchingP.getPhi();
      dphi = TVector2::Phi_mpi_pi(dphiRaw);
      deta = mfttrack_on_matchingP.getEta() - muontrack_on_matchingP.getEta();
